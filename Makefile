...
kernel/kernel.bin: kernel/kernel.o gash/shell.o kernel/string.o fs/ramfs/ramfs.o mm/memory.o mm/paging.o drivers/audio.o drivers/keyboard.o drivers/usb.o drivers/graphics.o drivers/mouse.o drivers/disk.o drivers/gpu.o drivers/rtc.o kernel/window.o kernel/abs.o kernel/cpudelay.o kernel/syscall_dispatcher.o kernel/syscall_table.o kernel/execute.o kernel/process.o ipc/ipc.o kernel/panic.o kernel/idt.o kernel/interrupt.o drivers/vga.o fs/vfs/vfs.o drivers/pci.o security/aslr.o kernel/gdt.o kernel/tss.o kernel/keyboard_isr_wrapper.o kernel/pit_isr_wrapper.o kernel/privileges.o kernel/vm86.o kernel/enter_user_mode.o kernel/ring3.o kernel/software_isr_wrapper.o drivers/serial.o kernel/gpf_isr_wrapper.o gash/mandelbrot.o security/rdrand32.o
...
gash/mandelbrot.o: gash/mandelbrot.c
	$(CC) $(DEBUG) $(ARCH) $(WARNINGS) -ffreestanding -fno-stack-protector -c gash/mandelbrot.c -o gash/mandelbrot.o
...
security/rdrand32.o: security/rdrand32.s
	$(AS) -32 -o security/rdrand32.o security/rdrand32.s
